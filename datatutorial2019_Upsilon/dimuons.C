...
void dimuon::FitPeak() {
  ...
  const Int_t nfitpar(11);
  TF1* f = new TF1("f",fitfun,_mmin,_mmax,nfitpar);
  f->SetParameters(100,9.46,0.1,100,10.02,0.1,100,10.35,0.1,0,0);
  hpeak->Fit("f");
  ...
  printf("\nFitResults:\n\tResonance mass: %5.3f +/- %5.3f GeV/c^2.\n",
	par[4],f->GetParErrors()[4]);
  printf("\nFitResults:\n\tResonance mass: %5.3f +/- %5.3f GeV/c^2.\n",
	par[7],f->GetParErrors()[7]);
  ...
  TF1 *signal1Fcn = new TF1("signalFcn",signal1,_mmin,_mmax,3);
  signal1Fcn->SetLineColor(kBlue);
  signal1Fcn->SetNpx(500);
  TF1 *signal2Fcn = new TF1("signalFcn",signal2,_mmin,_mmax,3);
  signal2Fcn->SetLineColor(kGreen);
  signal2Fcn->SetNpx(500);
  TF1 *signal3Fcn = new TF1("signalFcn",signal3,_mmin,_mmax,3);
  signal3Fcn->SetLineColor(kYellow);
  signal3Fcn->SetNpx(500);
  TF1 *backFcn = new TF1("backFcn",backgr,_mmin,_mmax,2);
  backFcn->SetLineColor(kGray);
  backFcn->SetLineStyle(2);

  signal1Fcn->SetParameters(par);
  signal1Fcn->Draw("same");
  signal2Fcn->SetParameters(&par[3]);
  signal2Fcn->Draw("same");
  signal3Fcn->SetParameters(&par[6]);
  signal3Fcn->Draw("same");
  
  backFcn->SetParameters(&par[9]);
  backFcn->Draw("same");
  ...
  legend->AddEntry(signal1Fcn,"#Upsilon(1s) fit","l");
  legend->AddEntry(signal2Fcn,"#Upsilon(2s) fit","l");
  legend->AddEntry(signal3Fcn,"#Upsilon(3s) fit","l");
  ...
  L.SetTextSize(0.03);
  L.DrawLatex(0.15,0.75,"resonance: #Upsilon(1s)");
  ...
  L.DrawLatex(0.15,0.60,"resonance: #Upsilon(2s)");
  L.DrawLatex(0.15,0.55,Form("mass: %5.3f #pm %5.3f GeV/c^{2}",
			     par[4], f->GetParErrors()[4]));
  L.DrawLatex(0.15,0.50,Form("width: %5.3f #pm %5.3f MeV/c^{2}", 
			     par[5]*1000, f->GetParErrors()[5]*1000));
  L.DrawLatex(0.15,0.45,"resonance: #Upsilon(3s)");
  L.DrawLatex(0.15,0.40,Form("mass: %5.3f #pm %5.3f GeV/c^{2}",
			     par[7], f->GetParErrors()[7]));
  L.DrawLatex(0.15,0.35,Form("width: %5.3f #pm %5.3f MeV/c^{2}", 
			     par[8]*1000, f->GetParErrors()[8]*1000));
...
Double_t fitfun(Double_t *x, Double_t *par) {
  ...
  return signal1(x,par) + signal2(x,&par[3]) + signal3(x,&par[6]) + backgr(x,&par[9]); 
}
