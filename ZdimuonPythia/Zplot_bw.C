...
void Zanalysis_bw()
{
   ...
   TH1D *hmu1pT = new TH1D("hmu1pT", "Simulated #mu^{-} p_{T} distribuition", 100, 0., 100.);
   ...
   TH1D *hmu2pT = new TH1D("hmu2pT", "Simulated #mu^{+} p_{T} distribuition", 100, 0., 100.);
   ...
   for (auto i = 0; i != nevt; ++i) {
      t->GetEntry(i);
      mass = Sqrt(Power(E1 + E2,2) - Power(px1 + px2,2) - Power(py1 + py2,2) - Power(pz1 + pz2,2));
      hZm->Fill(mass);
      if (mass >= 80 || mass <= 100)
         hZmcut->Fill(mass);
      hmu1pT->Fill(pT1);
