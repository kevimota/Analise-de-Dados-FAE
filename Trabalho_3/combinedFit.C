...
void combinedFit() { 
  ...
  // genetic does not work with fixed parameters : set very small range 
  fitter.Config().ParSettings(4).SetLimits(29.99,30.01);
  ...
  fitter.Config().ParSettings(5).SetLimits(0,10);
  ...
  hSB->Draw(); 
  TPaveLabel *p = new TPaveLabel(0.7,0.2,0.9,0.3,Form("CPU = %g s",cputime),"brNDC");
  p->Draw();
